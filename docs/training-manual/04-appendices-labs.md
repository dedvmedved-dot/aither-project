...
digraph ChatFlow {
    ...
    nginx1 [label="VPS1 nginx\n:10443 TLS", shape=box, style="filled", fillcolor="#e3f2fd", color="#1976d2"]
    nginx2 [label="VPS2 nginx\n:80", shape=box, style="filled", fillcolor="#f3e5f5", color="#7b1fa2"]
    bff [label="BFF :3000\nNode.js", shape=box, style="filled", fillcolor="#fce4ec", color="#e91e63"]
    gw [label="Gateway\n:30900", shape=box, style="filled", fillcolor="#fff3e0", color="#ff9800"]
    vllm [label="vLLM\n:8000", shape=box, style="filled", fillcolor="#e8f5e9", color="#43a047"]

    browser -> nginx1 [label="1. HTTPS"]
    nginx1 -> nginx2 [label="2. WireGuard", style=dashed, color="#43a047"]
    nginx2 -> bff [label="3. :3000"]
    bff -> nginx1 [label="4. :30900", style=dashed, color="#e91e63"]
    nginx1 -> gw [label="5. NodePort", style=dotted, color="#1976d2"]
    gw -> vllm [label="6. ClusterIP"]
    vllm -> gw [label="7. SSE-поток", style=dashed, color="#43a047"]
    gw -> bff [label="8. SSE", style=dashed]
    bff -> browser [label="9. SSE\n(токен за токеном)", style=dashed, color="#e91e63"]
}
```
...
digraph BillingFlow {
    ...
    redis [label="1. Redis\nпроверка RPM/TPM\nINCR rpm:{org}:{window}", shape=cylinder, style="filled", fillcolor="#fce4ec", color="#e91e63"]
    vllm [label="2. vLLM\nинференс\nвозвращает usage", shape=box, style="filled", fillcolor="#e8f5e9", color="#43a047"]
    pg [label="3. PostgreSQL\nUPDATE billing_accounts\nSET balance=balance-N", shape=cylinder, style="filled", fillcolor="#f3e5f5", color="#9c27b0"]

    gw -> redis -> gw -> vllm -> gw -> pg
}
```
...
digraph OAuthFlow {
    ...
    browser [label="Браузер", shape=box, style="filled", fillcolor="#e3f2fd", color="#1976d2"]
    bff [label="BFF :3000", shape=box, style="filled", fillcolor="#fce4ec", color="#e91e63"]
    provider [label="Провайдер\n(GitHub/Google/VK)", shape=box, style="filled", fillcolor="#e8f5e9", color="#43a047"]
    pg [label="Portal DB", shape=cylinder, style="filled", fillcolor="#f3e5f5", color="#9c27b0"]

    browser -> bff [label="1. GET /auth/github"]
    bff -> browser [label="2. Redirect"]
    browser -> provider [label="3. Логин"]
    provider -> browser [label="4. Redirect + code"]
    browser -> bff [label="5. callback?code="]
    bff -> provider [label="6. code→token"]
    provider -> bff [label="7. user info"]
    bff -> pg [label="8. INSERT users"]
...
digraph CICDFlow {
    ...
    push [label="1. git push\n(main, gateway/**)", shape=box, style="filled", fillcolor="#e3f2fd", color="#1976d2"]
    actions [label="2. GitHub Actions\ncheckout → docker build", shape=box, style="filled", fillcolor="#fff3e0", color="#ff9800"]
    registry [label="3. ghcr.io\ndocker push", shape=cylinder, style="filled", fillcolor="#f3e5f5", color="#9c27b0")]
    deploy [label="4. SSH → VPS2\nkubectl set image", shape=box, style="filled", fillcolor="#fce4ec", color="#e91e63"]
    health [label="5. Health check\ncurl /health (×5)", shape=box, style="filled", fillcolor="#e8f5e9", color="#43a047"]
    tg [label="6. Telegram\n✅ Deployed", shape=box, style="filled", fillcolor="#e8f5e9", color="#43a047"]

    push -> actions -> registry -> deploy -> health -> tg
}
```
