# Aither Platform — руководство пользователя

**Дата:** 07.07.2026  
**Версия:** v2.0  
**URL портала:** https://fb1.spb.ru:10443  
**API URL:** https://fb1.spb.ru:10443/api/v1
...
1. Откройте https://fb1.spb.ru:10443 в браузере
...
**Эндпоинт:**
```
POST https://fb1.spb.ru:10443/api/v1/chat/completions
```
...
*Статус: ✅ работает — гибридный поиск (Wiki Graph + ChromaDB)*

### Как работает RAG

RAG (Retrieval-Augmented Generation) обогащает ответы модели знаниями из базы:

```
Вопрос → Wiki Graph (keyword search, 8 страниц) + ChromaDB (векторный поиск, 39 чанков)
       → релевантные фрагменты → добавляются в промпт → модель отвечает с учётом контекста
```

### Как включить RAG в чате

1. Откройте чат в портале
2. Включите переключатель **RAG** (рядом с полем ввода)
3. Задайте вопрос — модель ответит с учётом базы знаний
4. Под ответом отобразятся **источники** (Wiki/ChromaDB) — нажмите чтобы посмотреть

### Что ищет RAG

- **Wiki Graph** — 8 страниц базы знаний: архитектура Gateway, безопасность, SIEM, Vault, ChromaDB, multi-tenant, аутентификация
- **ChromaDB** — 39 чанков учебного пособия Aither (24 главы, ~780 стр.)

### Примеры запросов

| Запрос | Что найдёт |
|---|---|
| «Какие лабораторные работы есть в учебнике?» | ChromaDB — список ЛР из пособия |
| «Как работает AI Security Gateway?» | Wiki Graph — страница ai-gateway.md |
| «Расскажи про multi-tenant архитектуру» | Wiki Graph — страница multi-tenant-arch.md |
...
| Модель | Сервер | GPU | max_tokens | Для чего |
|---|---|---|---|---|
| **Qwen 2.5 14B** | n8 | 2× RTX 6000 (TP=2) | 4096 | Быстрые ответы, код, простые задачи |
| **Qwen 2.5 32B** | n7 | 2× RTX 6000 (TP=2) | 8192 | Сложный анализ, длинные тексты, рассуждения |
| **Qwen 2.5 Coder 14B** | n7 | 2× RTX 6000 (shared) | 4096 | Генерация кода, отладка, рефакторинг |
...
- **Coder 14B** — для программирования: написание функций, отладка, code review, рефакторинг
...
1. Откройте https://fb1.spb.ru:10443
...
1. Откройте чат, включите переключатель **RAG**
2. Спросите: «Какие лабораторные работы есть в учебнике Aither?»
3. **Ожидаемый результат:** модель отвечает с учётом учебного пособия, под ответом — список источников

### Сценарий 10: Grafana

1. Откройте http://fb1.spb.ru:10443/grafana
...
### Сценарий 11: Мульти-провайдерный OAuth
...
|---|---|---|
| OAuth-вход (3 провайдера) | ✅ | https://fb1.spb.ru:10443 |
...
| Grafana (GPU-метрики) | ✅ | fb1.spb.ru:10443/grafana |
| RAG (гибридный: Wiki + ChromaDB) | ✅ | Переключатель RAG в чате |
| ЮKassa (боевой режим) | ⬜ | В планах |
| Parsec на n7 | ⬜ | В планах |

---

## 11. LDAP-аутентификация (Enterprise)

Для организаций, использующих корпоративный LDAP/FreeIPA/ALD Pro, доступна LDAP-аутентификация.

### Как работает

1. Администратор включает LDAP в настройках организации
2. При входе пользователь выбирает «Войти через LDAP»
3. BFF проверяет учётные данные через LDAP-сервер
4. При успехе — выпускается JWT-токен портала

### Требования

- LDAP-сервер должен быть доступен с VPS2
- Учётные данные: `uid` + пароль
- Поддерживаются: FreeIPA, ALD Pro, OpenLDAP

---

## 12. Профили безопасности организаций

Администратор организации может настроить политики безопасности:

| Параметр | Описание | По умолчанию |
|---|---|---|
| Чат | Разрешить/запретить чат-интерфейс | Разрешён |
| Внешние модели | Доступ к внешним API-моделям | Запрещён |
| DLP-фильтры | Блокировка утечек данных | Включены |
| Prompt Injection | Защита от инъекций | Включена |
| Rate Limit (RPM) | Лимит запросов/мин | 60 |
| Rate Limit (TPM) | Лимит токенов/мин | 100 000 |
| Макс. модель | Самая мощная доступная модель | 32B |
| Экспорт данных | Разрешить экспорт истории | Запрещён |
| Аудит-лог | Вести журнал всех запросов | Включён |
| Инвайт-код | Код для регистрации в организации | aither-2026 |
| Max Tokens/запрос | Лимит токенов на один запрос | 4096 |
| Retention (дни) | Хранение истории чатов | 90 |
| API-ключи | Разрешить создание API-ключей | Разрешено |
| LDAP | Включить LDAP-аутентификацию | Выключено |

---

## 13. Внешний API (OpenAI-совместимый)

Платформа предоставляет OpenAI-совместимый API для интеграции с любыми инструментами.
...
- **Полная совместимость с OpenAI API** (формат запросов/ответов)
- **SSE-стриминг** — поддержка `stream: true`
- **Аутентификация:** `Bearer` токен (`ak-` префикс)
- **Ограничения:** Rate Limit согласно политике организации
- **Безопасность:** DLP + Prompt Injection на Gateway
- **Биллинг:** автоматическое списание токенов

### OpenAPI-спецификация

Swagger-спецификация доступна в `docs/openapi.yaml` репозитория.
...
**JavaScript:**
```javascript
const response = await fetch("https://fb1.spb.ru:10443/api/v1/chat/completions", {
  method: "POST",
  headers: {
...
| Проблема | Решение |
|---|---|
| Не входит через OAuth | Очистить кэш браузера, попробовать другого провайдера |
| Модель не отвечает | Проверить баланс организации, попробовать другую модель |
| 401 Unauthorized | Проверить API-ключ (не отозван?) |
| 402 Insufficient Balance | Дождаться авто-пополнения или пополнить через администратора |
| 403 Forbidden | Запрос заблокирован DLP или содержит инъекцию — переформулировать |
| 429 Rate Limit | Слишком много запросов — подождать минуту |
| Gateway недоступен | Обратиться к администратору платформы |

---

## 15. Организации и биллинг (новое в v1.1)

### Именование организаций

При регистрации автоматически создаётся личная организация с именем `{ВашеИмя}-организация`.
Дополнительные организации можно создать через портал.

### Баланс и тарифы

Каждая организация имеет:
- **Баланс токенов** — расходуется при использовании моделей
- **Тарифный план** — определяет лимиты (RPM, TPM, дневной лимит) и доступные модели
- **Стартовый баланс** — 100 000 токенов при регистрации
- **Авто-пополнение** — до 10 раз при исчерпании баланса

### Просмотр баланса

На портале: меню → **Профиль** → баланс организации.
В админ-панели: вкладка 🏢 **Организации** → таблица с балансом.

---

## 16. Администрирование (новое в v1.1)

Администраторы платформы имеют доступ к панели управления:
`https://fb1.spb.ru:10443/admin.html`

### Возможности администратора

| Действие | Где |
|---|---|
| Просмотр всех организаций | 🏢 Организации |
| Удаление организаций | 🏢 → 🗑 |
| Просмотр пользователей | 👥 Пользователи |
| Смена роли пользователя | 👥 → выпадающий список |
| Просмотр организаций пользователя | 👥 → 🏢 |
| Удаление пользователя | 👥 → 🗑 |
| Просмотр всех API-ключей | 🔑 API-ключи |
| Отзыв API-ключа | 🔑 → 🚫 Отозвать |
| Начисление/списание токенов | 💰 Токены |
| Смена тарифа организации | ⚙️ Тарифы |
| Настройка LDAP | 🔧 Настройки |

Полная документация: `docs/03-admin-guide.md`.

---

## 17. LDAP-аутентификация (новое в v1.1)

При настроенном LDAP пользователи могут входить под корпоративной учётной записью.
Роль в портале определяется членством в группах LDAP.

Настройка LDAP выполняется администратором через админ-панель (🔧 Настройки).
...
| API (чат) | `https://fb1.spb.ru:10443/v1/chat/completions` |
| API (модели) | `https://fb1.spb.ru:10443/v1/models` |
| Grafana | `https://fb1.spb.ru:10443/grafana` |
| Документация | `docs/` в репозитории `aither-project` |
