#!/usr/bin/env python3
"""Create billing_idempotency table and enhance gateway_idempotency. CHANGE-0022-C3."""
...
DDL = """
-- Idempotency table for billing operations — org-scoped with UNIQUE constraint
...
CREATE TABLE IF NOT EXISTS billing_idempotency (
    ...
    settle_result         VARCHAR(32),
    refund_result         VARCHAR(32),
    created_at            TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    completed_at          TIMESTAMPTZ,
    expiry                TIMESTAMPTZ NOT NULL DEFAULT (NOW() + INTERVAL '24 hours'),
    CONSTRAINT pk_billing_idempotency PRIMARY KEY (id),
    CONSTRAINT uq_billing_idempotency_org_key UNIQUE (organisation, idempotency_key)
);
...
CREATE INDEX IF NOT EXISTS idx_billing_idempotency_org_key ON billing_idempotency(organisation, idempotency_key);
CREATE INDEX IF NOT EXISTS idx_gateway_idempotency_expiry ON gateway_idempotency(expiry);
"""

cur.execute(DDL)
conn.commit()
print("Tables created/updated successfully (CHANGE-0022-C3)")
...
cur.execute("""
    ...
    WHERE rel.relname = 'billing_idempotency'
    ORDER BY con.contype, con.conname
""")
print("\nbilling_idempotency constraints:")
for r in cur.fetchall():
    print(f"  {r[0]} ({r[1]}): {r[2]}")
