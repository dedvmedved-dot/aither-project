...
def base_task(**updates):
    value = {
        ...
        'max_commits': 1, 'require_clean_start': True, 'require_clean_finish': True,
        'architect_acceptance_required': True, 'validation_commands': [],
...
class FakeRun:
    def __init__(self, outputs=None, failures=None):
        self.outputs = outputs or {}; self.failures = failures or set(); self.calls = []
    def __call__(self, argv, cwd):
        key = tuple(argv); self.calls.append((list(argv), Path(cwd)))
        if key in self.failures: raise subprocess.CalledProcessError(1, argv)
...
class HostRunnerTests(unittest.TestCase):
    ...
    def test_agent_exec_required_boolean(self):
        ...
        with self.assertRaises(runner.RunnerError): runner.validate_task(task)
        with self.assertRaises(runner.RunnerError): runner.validate_task(base_task(capabilities={**base_task()['capabilities'], 'agent_exec': 'yes'}))
    def test_host_capabilities_required(self):
        task = base_task(); del task['host_capabilities']['push']
        with self.assertRaises(runner.RunnerError): runner.validate_task(task)
    def test_validation_commands_are_argv_lists(self):
        with self.assertRaises(runner.RunnerError): runner.validate_task(base_task(validation_commands=['shell command']))
    def test_exact_codex_argv(self):
        os.environ.pop('CODEX_BIN', None)
        self.assertEqual(runner.build_codex_argv(), ['codex','--ask-for-approval','never','exec','--sandbox','workspace-write',runner.FIXED_PROMPT])
    ...
    def test_unauthorized_path_rejected(self):
        with self.assertRaises(runner.RunnerError): runner.validate_changed_paths(['bad.txt'],base_task())
    def test_architect_tamper_rejected(self):
        with self.assertRaises(runner.RunnerError): runner.validate_changed_paths(['.agent/CURRENT_TASK.md'],base_task(allowed_paths=['impl.txt','.agent/CURRENT_TASK.md']))
    def test_source_write_false_rejects_changes(self):
        caps={**base_task()['capabilities'],'source_write':False}
        with self.assertRaises(runner.RunnerError): runner.validate_changed_paths(['impl.txt'],base_task(capabilities=caps))
    ...
    def test_baseline_equality_not_required(self):
        fake=FakeRun(); runner.verify_baseline_ancestor(Path('/repo'),'base','later',fake)
