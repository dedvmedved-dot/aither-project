...
INJECTION_PATTERNS = [
    ...
    # Jailbreak
    r"(DAN|developer\s*mode|god\s*mode)\s*(mode|enabled|activated)",
...
DLP_PATTERNS = [
    ...
    (r"\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9]{2})[0-9]{12})\b",
     "credit_card_number"),
    # Russian passport (серия + номер)
    (r"\b\d{2}\s?\d{2}\s?\d{6}\b", "possible_passport"),
    # SSN (US)
    (r"\b\d{3}-\d{2}-\d{4}\b", "ssn"),
    # Phone numbers (Russian)
    (r"(?<!\w)(?:\+7|8)[\s\-]?\(?\d{3}\)?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}(?!\w)", "phone_number"),
    # Email addresses
