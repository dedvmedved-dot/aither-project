// LDAP authentication for Aither Portal
// Supports FreeIPA, ALD Pro, OpenLDAP, Active Directory
...
function bindService(client: any): Promise<void> {
  return new Promise((resolve, reject) => {
    if (!LDAP_BIND_DN) {
      // Anonymous bind (not recommended, but works for some setups)
      client.bind("", "", (err: any) => {
        if (err) return reject(new Error("LDAP anonymous bind failed: " + err.message));
        resolve();
      });
      return;
    }
    client.bind(LDAP_BIND_DN, LDAP_BIND_PASSWORD, (err: any) => {
      if (err) return reject(new Error("LDAP service bind failed: " + err.message));
      resolve();
    });
  });
}
...
// --- Search for user DN ---
function searchUser(client: any, username: string): Promise<{ dn: string; attrs: Record<string, any> } | null> {
  return new Promise((resolve, reject) => {
    ...
    client.search(LDAP_USER_BASE, opts, (err: any, res: any) => {
      if (err) return reject(new Error("LDAP search failed: " + err.message));

      let found: any = null;
      res.on("searchEntry", (entry: any) => {
        const attrs: Record<string, any> = {};
        entry.pojo.attributes.forEach((a: any) => {
...
function bindUser(client: any, dn: string, password: string): Promise<void> {
  return new Promise((resolve, reject) => {
    client.bind(dn, password, (err: any) => {
      if (err) return reject(new Error("LDAP user bind failed: invalid credentials"));
      resolve();
    });
  });
}
...
function getUserGroups(client: any, userDn: string): Promise<string[]> {
  return new Promise((resolve) => {
    ...
    client.search(LDAP_GROUP_BASE, opts, (err: any, res: any) => {
      if (err) return resolve([]);
      const groups: string[] = [];
      res.on("searchEntry", (entry: any) => {
        const cn = entry.pojo.attributes.find((a: any) => a.type === "cn");
        if (cn && cn.values[0]) groups.push(cn.values[0]);
      });
...
// --- Main export: authenticate via LDAP ---
