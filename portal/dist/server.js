  ...
  `);
    ...
    app.post("/api/v1/orgs/:orgId/api-keys", async (req, reply) => {
       ...
       VALUES ($1,$2,$3,$4) RETURNING key_id, api_key_prefix, name, status, created_at`, [orgId, apiKey, apiKeyPrefix, name]);
        ...
        k8sPool.query(`INSERT INTO portal_api_keys (org_id, user_id, api_key, api_key_prefix, name)
       VALUES ($1,$2,$3,$4,$5) ON CONFLICT (api_key) DO NOTHING`, [orgId, p.user_id, apiKey, apiKeyPrefix, name]).catch(e => console.log("[k8s-replicate] create key error:", e.message));
    ...
    app.delete("/api/v1/orgs/:orgId/api-keys/:keyId", async (req, reply) => {
        ...
        // Replicate to K8s PG
        k8sPool.query("UPDATE portal_api_keys SET status='revoked' WHERE key_id=$1", [keyId]).catch(e => console.log("[k8s-replicate] revoke key error:", e.message));
        return { key: r.rows[0] };
    });
    ...
    app.post("/api/v1/chat/completions", async (req, reply) => {
        ...
        const keyRow = await pool.query("SELECT org_id, name FROM portal_api_keys WHERE api_key=$1 AND status='active' AND (expires_at IS NULL OR expires_at > now())", [apiKey]);
        ...
        const delegationToken = jsonwebtoken_1.default.sign({ org_id, key_id: "api" }, DELEGATION_PRIVATE_KEY, { algorithm: "RS256", expiresIn: "5m", issuer: "aither-api" });
        // Read request body
        const body = req.body || {};
        if (!body.messages) {
    ...
    app.post("/api/v1/chats/:chatId/messages", async (req, reply) => {
        ...
        const secResult = (0, security_1.checkSecurity)([{ role: "user", content }]);
        if (!secResult.ok) {
            const ip = (req.headers["x-forwarded-for"] || req.headers["x-real-ip"] || req.ip || "").toString();
            await pool.query(`INSERT INTO security_audit (user_id, org_id, chat_id, category, reason, content_snippet, model, ip_address)
         VALUES ($1,$2,$3,$4,$5,$6,NULL,$7)`, [p.user_id, org_id || null, chatId, secResult.category, secResult.reason,
