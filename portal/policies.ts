...
export interface OrgPolicy {
  ...
  sensitive_data_patterns: string[];    // custom regex patterns for DLP

  // Access control
  allowed_ip_cidrs: string[];           // IP whitelist for API access ([] = any)
  mfa_required: boolean;
  session_timeout_min: number;          // JWT session timeout

  // API key policies
  api_key_max_age_days: number | null;  // null = no expiration
  api_key_rotation_required: boolean;

  // Rate limits (override tier defaults, null = use tier)
  custom_rpm: number | null;
  custom_tpm: number | null;
  max_concurrent_requests: number | null;

  // Models & tokens
  allowed_models: string[];             // [] = all models
  max_tokens_per_request: number | null; // null = no limit
...
export async function loadPolicy(pool: Pool, orgId: string): Promise<OrgPolicy> {
  const r = await pool.query(
    "SELECT * FROM portal_org_policies WHERE org_id = $1", [orgId]);
...
export async function savePolicy(pool: Pool, orgId: string, policy: Partial<OrgPolicy>): Promise<OrgPolicy> {
...
export function validatePolicy(policy: Partial<OrgPolicy>): string | null {
