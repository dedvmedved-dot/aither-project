...
*(выполняется в фоновой задаче — будет дополнено)*

---

## 6. Выполнение критериев приёмки

| № | Критерий | Статус | Примечание |
|---|---|---|---|
| 1 | **14B: Running, Ready 1/1, Restarts 0** | ✅ **ВЫПОЛНЕН** | |
| 2 | **32B: Running, Ready 1/1, Restarts 0** | ✅ **ВЫПОЛНЕН** | |
| 3 | Обе модели дают осмысленные ответы | 🟡 **Частично** | Completion — да, Chat 32B — нет |
| 4 | Минимум 10 smoke-тестов проходят | 🟡 | 8 из 10 пройдены по completion |
| 5 | 30–60 минут нагрузки без OOM | 🟡 | Нет данных (не проводилось) |
| 6 | Нет потерянных CUDA-процессов | 🟡 | Ожидается orphan-тест |
...
**Inference-сервисы — готовы на 60%.**

| Компонент | Оценка |
|---|---|
| Kubernetes-инфраструктура | 🟢 9/10 |
| Безопасность | 🟢 9/10 |
| Сетевая изоляция | 🟡 4/10 (Flannel) |
| 14B Instruct | 🟢 8/10 (работает, но с offload) |
| 32B GPTQ Chat | 🔴 3/10 (только completion) |
| Отказоустойчивость | 🔴 3/10 |
| Мониторинг | 🔴 1/10 |
