...
| `vllm-32b-gptq` | ✅ Running | 1/1 | 0 | n8 (10.244.0.139) | `--quantization gptq` возвращён. Completion: `"Paris..."` ✅ Chat: `!!!` (Base модель, ожидаемо) |

**Обе модели работают.** API стабилен (10/10 успешных запросов). n7 uncordoned.
...
| 5 | **n7 uncordoned** | ✅ | После подтверждения что 32B работает на n8 и API стабилен |
| 6 | **API stability check** | ✅ | 10/10 `kubectl get nodes` без ошибок. API стабилен |
...
| № | Действие | Статус |
|---|---|---|
| 7 | 5 циклов orphan-теста | ✅ Выполнено. Орфаны подтверждены на n7 |
| 8 | Load test 60 мин | 🔴 Не проведён (требуется стабильный API, который сейчас подтверждён — можно проводить) |
