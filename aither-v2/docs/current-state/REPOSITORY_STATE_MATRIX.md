...
|-------|-------------|---------------|----------------|
| Qwen2.5-32B-Instruct-AWQ manifest/configuration | `deploy/vllm-32b-instruct-awq.yaml` (commit `407cbe7`) | ✅ Confirmed | PARTIALLY CONFIRMED — Git confirms config; running deployment is runtime-reported |
| Qwen3-32B-AWQ manifest/configuration | `deploy/vllm-qwen3-32b-awq.yaml` (commit `8ba71ac`) | ✅ Confirmed | PARTIALLY CONFIRMED — Git confirms config; running deployment is runtime-reported |
