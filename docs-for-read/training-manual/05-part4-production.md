...
> **Цель раздела:** точная картина production-стенда Aither на июль 2026: хосты, сервисы, сеть, биллинг.
...
| **n7** | 10.129.13.77 | Astra SE 1.8 | K8s worker, vLLM 32B (systemd :8000) | 2× RTX 6000 |
| **n8** | 10.129.13.78 | Astra SE 1.8 | K8s control-plane, Gateway :8080, vLLM 14B (pod :30014), PostgreSQL :5432, Redis :6379 | 2× RTX 6000 |
| **Cisco** | 10.129.11.21 | Cisco 815 | Jump Host, VPN-концентратор | — |

## Б.2. Сервисы

| Сервис | Где | Порт | Технология |
|---|---|---|---|
| Gateway | n8 | :8080 | Python (systemd) |
| vLLM 14B | n8 (K8s pod) | :30014 | Qwen2.5-14B-Instruct, TP=2 |
| vLLM 32B | n7 (systemd) | :8000 | Qwen2.5-32B-GPTQ, TP=2, INT4 |
| Портал | VPS1 | :80/:3000 | Node.js (Fastify) + nginx |
| PostgreSQL | n8 | :5432 | PostgreSQL 16 (billing) |
| Redis | n8 | :6379 | Redis 7 (кэш tier'ов) |
| K8s | n8 | :6443 | K8s v1.33.5, 2 ноды Ready |

## Б.3. Сетевая топология

| Туннель | Между | Технология |
|---|---|---|
| VPS1 ↔ VPS2 | Публичный интернет | WireGuard (wg0) 10.99.0.0/24 |
| VPS2 → n7/n8 | Через Cisco | Cisco VPN (tun1) |
| n7 ↔ n8 | Локальная сеть | VLAN 308 10.129.13.0/24 |
| VPS3 → VPS2 | Публичный интернет | SSH + rsync (hermes-sync /30 мин) |

**Путь запроса:** Пользователь → VPS2:10443 (SSL) → WireGuard → VPS1:80 → BFF:3000 → Cisco VPN → Gateway n8:8080 → vLLM 14B/32B.
...
|---|---|---|
| Gateway | ✅ | n8:8080, systemd |
| vLLM 14B | ✅ | K8s pod на n8 |
| vLLM 32B | ✅ | systemd на n7 |
...
| VPS3 (резерв) | ✅ | hermes-sync /30 мин |

> ⚠️ **ChromaDB не работает.** Векторная база не поднята. Gateway использует wiki-graph (6 wiki-страниц) в режиме hybrid-query. Восстановление ChromaDB — задача ближайшего плана.
