...
![MVP архитектура](diagrams/architecture.svg)
...
![Биллинг и каталог](diagrams/billing-flow.svg)
...
## 4. Этап 3: Observability и безопасность (дни 11–14)
...
![Observability и безопасность](diagrams/chat-flow.svg)
...
Gateway: hybrid_rag.py
  ...
  └─ ChromaDB (векторный поиск)
        ...
        └─ Коллекция "textbook": 322 чанка учебника
              │
              ▼
...
![RAG архитектура](diagrams/day13-architecture.svg)
...
## 7. Этап 5: Продакшен-класс (недели 3–6)
...
![Продакшен-класс](diagrams/physical-architecture-v2.svg)
...
| 2. Биллинг + каталог | 5 | ✅ | Монетизация + мультимодельность |
| 2.5. Авто-баланс | 2 | ✅ | Стартовые токены + Grafana |
| 3. Observability + безопасность | 6 | ✅ | Мониторинг + DLP + mTLS |
| 4. RAG + кастомизация | 8 | ✅ | Гибридный RAG (Wiki+ChromaDB) + QLoRA |
| 4.5. Стабилизация портала | 7 | ✅ | OAuth fix, чат fix, org_id, баланс |
| 5. Продакшен-класс | 4 | 🔄 | Тарифы + failover + SaaS + LDAP |
| 5a. Требования руководства | 7 | ✅ | DLP egress + SIEM + Vault + LLM-Wiki |
| 6. Эксплуатация | 6 | ✅ | HPA + CI/CD + учебник (24 главы) |
| 7. Закрытый контур | 7 | ✅ | offline-deploy v1.2.0 |
| **Итого** | **59** | **55/59** | **93% готовность** |
