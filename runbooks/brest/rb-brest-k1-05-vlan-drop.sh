#!/bin/bash
# rb-brest-k1-05-vlan-drop — VLAN 308 isolation
# Часть ПМИ AIOps — испытания Aither + ПК СВ Брест
# Сгенерирован: 2026-07-15 14:00:47 MSK

set -e
LOG="/opt/aiops-aither/logs/runbooks/rb-brest-k1-05-vlan-drop.log"
...
log "=== START rb-brest-k1-05-vlan-drop ==="
log "Description: VLAN 308 isolation"
...
# Инжекция
log "Injecting fault..."
ssh mikrotik '/ip firewall filter add chain=forward src-address=10.129.13.0/24 action=drop'
...
# Восстановление
log "Recovering..."
ssh mikrotik '/ip firewall filter remove [find comment=test-drop]'
...
log "=== END rb-brest-k1-05-vlan-drop ==="
