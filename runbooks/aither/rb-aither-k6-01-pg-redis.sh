#!/bin/bash
# rb-aither-k6-01-pg-redis — PG + Redis simultaneous stop
# Часть ПМИ AIOps — испытания Aither + ПК СВ Брест
# Сгенерирован: 2026-07-15 14:00:47 MSK

set -e
LOG="/opt/aiops-aither/logs/runbooks/rb-aither-k6-01-pg-redis.log"
...
log "=== START rb-aither-k6-01-pg-redis ==="
log "Description: PG + Redis simultaneous stop"
...
# Инжекция
log "Injecting fault..."
ssh n8 'systemctl stop postgresql; systemctl stop redis'
...
# Восстановление
log "Recovering..."
ssh n8 'systemctl start postgresql; systemctl start redis'
...
log "=== END rb-aither-k6-01-pg-redis ==="
