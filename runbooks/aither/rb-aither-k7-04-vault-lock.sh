#!/bin/bash
# rb-aither-k7-04-vault-lock — Vault seal
# Часть ПМИ AIOps — испытания Aither + ПК СВ Брест
# Сгенерирован: 2026-07-15 14:00:47 MSK

set -e
LOG="/opt/aiops-aither/logs/runbooks/rb-aither-k7-04-vault-lock.log"
...
log "=== START rb-aither-k7-04-vault-lock ==="
log "Description: Vault seal"
...
# Инжекция
log "Injecting fault..."
ssh n8 'vault operator seal 2>/dev/null || echo vault-sealed'
...
# Восстановление
log "Recovering..."
ssh n8 'vault operator unseal 2>/dev/null || echo vault-unseal-skip'
...
log "=== END rb-aither-k7-04-vault-lock ==="
